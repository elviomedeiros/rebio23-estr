...
A relação entre a comunidade zooplanctônica e as variáveis ambientais foi avaliada por meio da Análise de Redundância (RDA), utilizando a matriz comunitária de densidade transformada e a matriz ambiental previamente selecionada [@legendre1998]. Antes da construção do modelo, as variáveis ambientais foram avaliadas pelo fator de inflação da variância (*VIF*), sendo mantidas aquelas com valores inferiores a 10 [@obrien2007]. Em seguida, foi realizada a seleção das variáveis ambientais pelo procedimento forward stepwise, permitindo identificar os preditores que melhor explicaram a variação na composição da comunidade [@blanchet2008].

A significância dos modelos de RDA foi avaliada por meio de testes de permutação, permitindo verificar a relação entre os preditores ambientais selecionados e a variação observada na comunidade zooplanctônica [@legendre1998].
